...
export default function Dashboard() {
    ...
    const toggleMode = async () => {
        ...
        try {
            await axios.post(`${API_URL}/settings/mode`, { mode: newMode });
            fetchData();
        } catch (e) { alert("Failed to switch mode"); }
    };

    const changeLeverage = async (e) => {
        const lev = parseInt(e.target.value);
        try {
            await axios.post(`${API_URL}/settings/leverage`, { leverage: lev });
            setAccount(prev => ({ ...prev, leverage: lev }));
        } catch (e) { alert("Failed to set leverage"); }
    };

    const closePosition = async (coin) => {
        if (!confirm(`Close ${coin} position?`)) return;
        try {
            await axios.post(`${API_URL}/trade/close/${coin}`);
            fetchData();
    ...
    const closeAll = async () => {
        ...
        try {
            await axios.post(`${API_URL}/trade/close-all`);
            fetchData();
        ...
        <div className="min-h-screen bg-slate-900 text-white p-6 font-sans">
            ...
            <header className="mb-6 flex flex-col md:flex-row justify-between items-center bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-lg gap-4">
                <div>
                    ...
                    <p className="text-slate-400 text-xs uppercase tracking-wider font-semibold">Williams %R + ML Strategy</p>
                </div>
