#!/usr/bin/env python3
"""
METRIC COMPARISON: BASELINE VS UPGRADE
Target: Jan 2 - Jan 23
Metrics: Win Rate, PnL, Sharpe, Sortino, Max Drawdown
"""
...
# Simulation
def simulate(run_name, tp_pct, sl_pct):
    trades = []
    ...
    # Sortino = Mean Return / Downside Std Dev
    downside_returns = profits[profits < 0]
    downside_std = np.std(downside_returns) if len(downside_returns) > 0 else 1
    sortino = np.mean(profits) / downside_std 
    
    print(f"\n📊 {run_name.upper()} REPORT")
    print(f"   Trades: {len(trades)}")
    print(f"   Win Rate: {win_rate:.2f}%")
    print(f"   Total Net PnL (Est): ${total_pnl:.2f}")
    print(f"   Avg Trade: ${np.mean(profits):.2f}")
    print(f"   Sharpe Ratio: {sharpe:.3f}")
    print(f"   Sortino Ratio: {sortino:.3f}")

print("\nRunning Comparison...")
simulate("Baseline (0.7/1.5)", 0.007, 0.015)
simulate("Upgrade (0.9/2.0)", 0.009, 0.020)
