#!/usr/bin/env python3
"""
NOVEMBER 2025 BACKTEST - FIXED MTF MERGE
Based on validated Dec/Jan methodology.
Data source: Binance (Nov 1 - Dec 1, 2025)
"""
...
def run_nov_backtest_fixed():
    print("=" * 80)
    print("🎯 NOVEMBER 2025 BACKTEST (Binance Data)")
    print("=" * 80)
    print("Period: Nov 1 - Dec 1, 2025")
    print("Strategy: Phase 2 (12 pos, 12h max, Compounding)")
    print("=" * 80)
    ...
    # Load 5m data
    df5_raw = pd.read_csv('/Users/alifiyaa/Downloads/quantEngineHyperliquid/MANDALORIAN_ENGINE/nov2025_binance_data.csv')
    df5_raw['timestamp'] = pd.to_datetime(df5_raw['timestamp'])
    df5_raw.set_index('timestamp', inplace=True)
    
    # Filter for November strictly just in case
    df5_raw = df5_raw[(df5_raw.index >= '2025-11-01') & (df5_raw.index < '2025-12-01')]
    ...
    if closed_trades:
        ...
        trades_df.to_csv('nov2025_fixed_mtf_results.csv', index=False)
        print(f"\n💾 Results saved to: nov2025_fixed_mtf_results.csv")

if __name__ == "__main__":
    run_nov_backtest_fixed()
