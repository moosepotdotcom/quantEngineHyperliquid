#!/usr/bin/env python3
"""
🛡️ ADAPTIVE SHIELD V2 - BACKTEST
Testing on Jan 2-17, 2026 live Hyperliquid data
Dynamic ATR-based thresholds + Mandalorian & AI filters
"""
...
# Adaptive Shield V2: Dynamic thresholds
BASE_THRESHOLD = 0.45
ATR_THRESHOLD = 70  # ATR value above which to increase threshold
ATR_PENALTY = 0.002  # 0.2% increase per ATR point above threshold
...
class BacktestTracker:
    ...
    def open_position(self, timestamp, direction, entry_price, confidence, atr, threshold):
        if self.position:
            return False
...
def run_backtest():
    print("=" * 80)
    print("🛡️ ADAPTIVE SHIELD V2 - BACKTEST")
    print("=" * 80)
    print(f"Period: {START_DATE} to {END_DATE}")
    print(f"Config: {LEVERAGE}x Leverage | {POSITION_SIZE_BTC} BTC")
    print(f"TP: {TP_PCT*100}% | SL: {SL_PCT*100}%")
    print(f"Dynamic Thresholds: Base {BASE_THRESHOLD*100}% + {ATR_PENALTY*100}% per ATR point > {ATR_THRESHOLD}")
    print("=" * 80)
    ...
    for idx, row in df_merged.iterrows():
        ...
        if closed_trade:
            ...
            print(f"{emoji} Trade #{len(tracker.trades)}: {closed_trade['direction']} | "
                  ...
                  f"ATR: {closed_trade['atr']:.1f} | Threshold: {closed_trade['threshold']*100:.1f}%")
        ...
        if not tracker.position:
            ...
            if direction:
                ...
                # Open position
                if tracker.open_position(timestamp, direction, row['close'], confidence, atr_val, required_threshold):
                    print(f"📍 Trade #{len(tracker.trades)+1}: OPEN {direction} @ ${row['close']:,.0f} | "
                          f"Confidence: {confidence*100:.1f}% | ATR: {atr_val:.1f} | Threshold: {required_threshold*100:.1f}%")
    ...
    if tracker.trades:
        ...
        # ATR analysis
        avg_atr = sum(t['atr'] for t in tracker.trades) / len(tracker.trades)
        avg_threshold = sum(t['threshold'] for t in tracker.trades) / len(tracker.trades)
        print(f"\nAvg ATR: {avg_atr:.1f}")
        print(f"Avg Threshold: {avg_threshold*100:.1f}%")
    ...
    if tracker.trades:
        ...
        trades_df.to_csv('adaptive_shield_v2_backtest.csv', index=False)
        print(f"\n💾 Results saved to: adaptive_shield_v2_backtest.csv")
