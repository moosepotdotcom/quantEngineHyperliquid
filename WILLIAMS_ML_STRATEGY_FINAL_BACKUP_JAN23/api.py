...
def get_market_status():
    ...
    try:
        ...
        account_info = {
            ...
            "filter_enabled": getattr(strategy, 'use_regime_filter', True),
            "balance": 0.0
        }
...
@app.post("/settings/filter")
def set_filter(settings: FilterSettings, current_user: User = Depends(get_current_admin)):
    if not strategy: return {"error": "Strategy initializing"}
    success = strategy.set_regime_filter(settings.enabled)
    return {"status": "ok", "filter_enabled": strategy.use_regime_filter}
...
@app.get("/history")
def get_trade_history(page: int = 1, limit: int = 10, db: Session = Depends(get_db)):
    from auth import Trade
    
    # Query DB
    total = db.query(Trade).count()
    trades = db.query(Trade).order_by(Trade.ts_exit.desc()).offset((page-1)*limit).limit(limit).all()
