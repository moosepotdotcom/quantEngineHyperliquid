...
class RenaissanceUnlimitedSimulator:
    def __init__(self):
    ...
    def run_unlimited_test(self, data_path, balance=1000.0, leverage=3.0, lot_size=0.02, tp_pct=0.005, sl_pct=0.004):
        print(f"🚀 Renaissance UNLIMITED Engine Execution: {data_path}")
        ...
        # 2. Simulation (UNLIMITED: Overlapping Trades Allowed)
        trades = []
        equity = balance
        ...
        for i in range(100, len(df)):
            ...
            if signal > 0:
                ...
                for idx_offset, row in enumerate(future.values):
                    # row mapping: [timestamp, open, high, low, close, volume, ...]
                    low_val = row[3]
                    high_val = row[2]
                ...
                if outcome != "NONE":
                    ...
                    equity += pnl # Note: Equity for unlimited can go very high due to overlapping pnl
                    trades.append({
        ...
        print(f"\n✅ UNLIMITED Execution Complete. Average Points: {res_df['points'].mean():.2f}")
        return res_df

if __name__ == "__main__":
    sim = RenaissanceUnlimitedSimulator()
    ...
    report = sim.run_unlimited_test(data_path, balance=initial_balance, tp_pct=tp, sl_pct=sl, lot_size=lot_size)
    ...
    if not report.empty:
        ...
        basename = os.path.basename(data_path).replace('.csv', '_unlimited.csv')
        out_path = os.path.join(results_dir, basename)
        report.to_csv(out_path, index=False)
        print(f"📊 Saved Unlimited Report: {out_path}")
