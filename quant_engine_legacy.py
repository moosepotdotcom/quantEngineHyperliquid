...
STRATEGY_PRESETS = {
    ...
    "High Win Rate Single": {
        "description": "Strict 4-5 trades/day. High WR. Single position.",
        "threshold": 1.2, # Ratio
        "use_hurst": True,
        "use_atr_penalty": True,
        "use_circuit_breaker": True,
...
class TradingEngine:
    ...
    def __init__(self):
        ...
        # Initialize Hyperliquid Info for shared use
        try:
            from hyperliquid.info import Info
            from hyperliquid.utils import constants
            self.info = Info(constants.MAINNET_API_URL, skip_ws=True)
            print("   ✅ Hyperliquid Info initialized (Shared)", flush=True)
        except Exception as e:
            print(f"   ⚠️ Could not initialize Hyperliquid Info: {e}", flush=True)
            self.info = None
        ...
        # Strategy State (Defaults)
        self.active_strategy = "V2 Enhanced"
        ...
        self.use_relative_strength = False
        ...
        self.whale_watcher = WhaleWatcher(threshold_usd=10000) # Lowered for testing
        self.liq_monitor = LiquidationMonitor(threshold_usd=50000)
        self.funding_monitor = FundingMonitor(info=self.info) # Share Info client
        
        self.whale_watcher.start()
        self.liq_monitor.start()
        if hasattr(self.funding_monitor, 'start'): self.funding_monitor.start()

        print("   📡 Specialized Agents ONLINE", flush=True)
        self.apply_strategy_preset(self.active_strategy)
        
        # V2 Brain
        print("   🤖 Initializing V2 Brain...")
        from strategies.specialized_agents.ml_scalper_v2 import MLScalperV2
        self.v2_scalper = MLScalperV2()
        
        self.logger = get_logger()
        self.elastic_managers = {
            'MTF': ElasticThresholdManager("MTF Scalper (5M)", 0.45, 0.45),
            'WH': ElasticThresholdManager("Winner Hunter (1H)", 0.32, 0.40)
        }
        
        # Legacy named managers for backward compatibility in logic
        self.mtf_elastic = self.elastic_managers['MTF']
        self.wh_elastic = self.elastic_managers['WH']

        # CircuitBreaker is defined locally in this file at line 114
        ...
        # Rate Limit / Caching
        self._fetch_cache = {} # (interval, limit) -> (timestamp, df)
        self._cache_ttl = 30 # 30 seconds
        
        print(f"   ✅ All systems initialized | Strategy: {self.active_strategy}", flush=True)
    ...
    def apply_strategy_preset(self, name):
        ...
        self.use_adx_filter = preset.get("use_adx_filter", False)
        self.use_relative_strength = preset.get("use_relative_strength", False)
        ...
        print(f"🛡️ Hurst: {'ON' if self.use_hurst else 'OFF'} | ATR: {'ON' if self.use_atr_penalty else 'OFF'} | ADX: {'ON' if self.use_adx_filter else 'OFF'}")
    ...
    def fetch_data(self, interval='1h', limit=500):
        ...
        if cache_key in self._fetch_cache:
            ts, df = self._fetch_cache[cache_key]
            if time.time() - ts < self._cache_ttl:
                print(f"      📡 Serving {interval} from cache ({int(time.time()-ts)}s old)")
                return df.copy()
        ...
        try:
            ...
            # 2. Update Cache
            self._fetch_cache[cache_key] = (time.time(), df.copy())
    ...
    def check_mtf_scalper(self):
        ...
        if hasattr(self, 'use_v2') and self.use_v2:
            df_5m = self.fetch_data('5m', 500)
            df_15m = self.fetch_data('15m', 500)
            df_30m = self.fetch_data('30m', 500)
            
            if df_5m is None or df_15m is None or df_30m is None:
                print(f"   ⚠️ V2 Redirect: Missing data (5m:{df_5m is not None}, 15m:{df_15m is not None}, 30m:{df_30m is not None})", flush=True)
                return None, 0.0
                
            # --- V2 ALPHA FEATURE ENGINEERING ---
            # Handled internally by v2_scalper.check_signal using MTFFeatureGenerator (231 features)
            ...
            signal_str = self.v2_scalper.check_signal(df_5m, df_15m, df_30m)
