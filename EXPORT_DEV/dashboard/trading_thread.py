...
class TradingThread(threading.Thread):
    ...
    def run(self):
        ...
        while self.running:
            ...
            try:
                ...
                # 2. Evaluate Alpha Strategies (only if no position)
                if not has_active_position:
                    ...
                    for sig in alpha_signals:
                        ...
                        if self.engine.live_trader:
                            success = self.engine.live_trader.execute_signal(payload)
                            if success:
                                has_active_position = True  # Mark position as active
                                break  # Stop checking for more signals
                ...
                if not has_active_position:
                    ...
                    if self._scan_counter == 0:  # Log once per minute
            ...
            # Sleep logic
            # Sleep logic
            time.sleep(12) # 12s polling (Sustainable for 5m scalper)
    ...
    def get_whale_walls(self):
        ...
        # Allow radar even in paper mode
        try:
            # 1. Fetch L2
            # --- L2 CACHE (Throttled to 15s to avoid 429s) ---
            if hasattr(self, '_l2_cache'):
                last_ts, last_snap = self._l2_cache
                if time.time() - last_ts < 15:
                    snapshot = last_snap
                else:
                    # Use engine's shared Info client
                    snapshot = self.engine.info.l2_snapshot("BTC")
                    self._l2_cache = (time.time(), snapshot)
            else:
                snapshot = self.engine.info.l2_snapshot("BTC")
                self._l2_cache = (time.time(), snapshot)
            ...
            if not levels or len(levels) < 2: 
                # Still try to return movements
                movements = []
                if hasattr(self.engine, 'whale_watcher'):
                    movements = self.engine.whale_watcher.recent_movements
                return {'l2_walls': [], 'l2_pops': [], 'movements': movements}
            ...
            # --- MOVEMENTS (Active AggTrades from Binance) ---
            movements = []
            if hasattr(self.engine, 'whale_watcher'):
                movements = self.engine.whale_watcher.recent_movements
                if movements:
                    print(f"📡 [DEBUG] Radar found {len(movements)} movements", flush=True)
            ...
            return {
                'l2_walls': walls[:10], 
                'l2_pops': pops[:5],
                'movements': movements # Binance active trades
            } 
        except Exception as e:
            if hasattr(self, 'add_log'):
                self.add_log(f"⚠️ Whale Scan Error: {e}")
            # Return movements even if L2 fails
            movements = []
            if hasattr(self.engine, 'whale_watcher'):
                movements = self.engine.whale_watcher.recent_movements
            return {'l2_walls': [], 'l2_pops': [], 'movements': movements}
        
        # Default Fallback (Always include movements if available)
        movements = []
        if hasattr(self.engine, 'whale_watcher'):
            movements = self.engine.whale_watcher.recent_movements
        return {'l2_walls': [], 'l2_pops': [], 'movements': movements}
