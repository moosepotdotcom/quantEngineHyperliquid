...
class HyperliquidTrader:
    ...
    def get_account_info(self) -> Dict:
        ...
        try:
            ...
            print(f"Error: {e}")
            return {'balance': 0, 'positions': [], 'margin_used': 0, 'total_ntl_pos': 0}

    def get_funding_rate(self, symbol: str) -> float:
        """Get current hourly funding rate for symbol"""
        try:
            ...
            if idx != -1 and idx < len(asset_ctxs):
                funding = float(asset_ctxs[idx].get('funding', 0.0))
                return funding
            return 0.0
        except Exception as e:
            print(f"⚠️ Error fetching funding: {e}")
            return 0.0
    ...
    def close_all_positions(self, close_type: str = 'market'):
        """
        Close all positions found on the exchange.
        type: 'market' (Immediate) or 'limit' (Mid-price)
        """
        print(f"\n🚨 CLOSING ALL POSITIONS (Type: {close_type.upper()})")
        ...
        if not positions:
            ...
            return {'status': 'processed', 'closed_count': 0}

        closed_count = 0
        
        # Get Mids for Limit Closes
        mids = {}
        if close_type == 'limit':
            try:
                mids = self.info.all_mids()
            except Exception as e:
                print(f"⚠️ Failed to fetch mids, falling back to market: {e}")
                close_type = 'market'

        for pos in positions:
            # pos structure: {'coin': 'BTC', 'szi': '1.0', ...}
            symbol = pos['coin']
            size_str = pos['szi']
            size = float(size_str)
            ...
            # To Close: If Long, Sell (is_buy=False). If Short, Buy (is_buy=True).
            close_side_is_buy = not is_long
            
            print(f"🛑 Closing {symbol} {'LONG' if is_long else 'SHORT'} ({abs_size})")
            
            try:
                if close_type == 'market':
                    self.exchange.market_open(symbol, close_side_is_buy, abs_size)
                    
                elif close_type == 'limit':
                    mid_px = float(mids.get(symbol, 0))
                    if mid_px == 0:
                        print(f"⚠️ No mid price for {symbol}, using market.")
                        self.exchange.market_open(symbol, close_side_is_buy, abs_size)
                    else:
                        price = mid_px
                        # Adjust price slightly to be "Maker" or "Taker"? 
                        # User asked for "Mid Price".
                        # Format price to significant figures? SDK handles it? 
                        # We usually need to round logic.
                        # For simple implementation, pass raw float, SDK *should* handle or we round.
                        # Hyperliquid usually likes string or rounded float.
                        # Let's use round(price, 5) for safety.
                        price = round(price, 1 if price > 1000 else 4)
                        
                        print(f"   📉 Placing Limit at ${price}")
                        self.exchange.order(symbol, close_side_is_buy, abs_size, price, {"limit": {"tif": "Gtc"}}, reduce_only=True)
        ...
        return {'status': 'processed', 'closed_count': closed_count}
