#!/usr/bin/env python3
"""
REGIME SHIELD PORTFOLIO BACKTEST - STRATEGY V1
Target: Jan 2 - Jan 23
Model: model_v1.json (5 Features)
Config: Period 21 | TP 0.7% | SL 1.5% | Conf > 0.65
Data: Hyperliquid History (Best Effort)
"""
...
print("\n" + "="*70)
print("🛡️ REGIME SHIELD PORTFOLIO BACKTEST - STRATEGY V1")
print("Target: Jan 2 - Jan 23 | Conf > 0.65")
print("Coins: BTC, ETH, SOL, AVAX, SUI")
print("="*70)

# 1. LOAD MODEL (model_v1.json)
print("\n📦 Loading Strategy V1 Model...")
v1_path = '../WILLIAMS_ML_STRATEGY_V1/model_v1.json'
v2_path = 'model_v1.json'
model_path = v1_path if os.path.exists(v1_path) else v2_path

if not os.path.exists(model_path):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(script_dir, '..', 'WILLIAMS_ML_STRATEGY_V1', 'model_v1.json')
    if not os.path.exists(model_path):
...
print(f"\n🔄 Outputting Portfolio Logs used Hyperliquid V1 Data...")
...
for coin in COINS:
    ...
    print(f"   ✅ Data: {len(df)} candles ({df['timestamp'].min()} - {df['timestamp'].max()})")
    ...
    for i, row in df.iterrows():
        ...
        if signal:
            ...
            if is_valid:
                ...
                current_position = {'coin': coin, 'time': timestamp, 'dir': signal, 'price': px, 'tp': tp, 'sl': sl, 'regime': regime}
            else:
                shield_stats[regime] += 1
                
    print(f"   📊 {coin}: {len(trades)} trades (Shield Blocked: {sum(shield_stats.values())})")
...
print(f"\n" + "="*70)
print(f"🏆 PORTFOLIO RESULTS (Strategy V1 + Regime Shield)")
print(f"Total Trades: {total} (Across {len(COINS)} Coins)")
...
if total > 0:
    ...
    print(df_trades[['coin', 'time', 'dir', 'regime', 'outcome']].tail(20).to_string())
