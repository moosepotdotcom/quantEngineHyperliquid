...
def run_micro_simulation():
    print("💎 GEM SNIPER: MICRO STRATEGY (TARGET 400 PTS)")
    print("="*70)
    ...
    # 2. Filter Date Range (Jan 2 - Jan 7)
    ...
    print(f"\n🔍 VERIFYING MICRO SNIPER (ATR < {limit})")
        ...
        if signal:
            ...
            for f_idx, f_row in future_data.iterrows():
                ...
                if signal == 'LONG':
                    ...
                    if l <= sl: 
                        outcome = "LOSS"
                        points = -STOP_POINTS # Negative points for loss
                        break
                else:
                    if l <= tp: 
                        outcome = "WIN"
    ...
    print("\n" + "="*70)
    print(f"📊 MICRO SNIPER RESULTS (Jan 2 - Jan 7)")
