...
def run_december_verification():
    print("💎 GEM SNIPER: DECEMBER 2025 VERIFICATION (The Ultimate Test)")
    print("="*70)
    
    engine = TradingEngine()
    # Fetch 15,000 candles to cover Dec 1 - Jan 11 (~42 days)
    # 42 * 24 * 12 = ~12,096. 15,000 is safe.
    print("🌐 Fetching deep history (15,000 candles)... this may take a moment.")
    df_raw = engine.fetch_data('5m', 15000)
    ...
    # Context (Also need deep history)
    df_15m = add_all_indicators(engine.fetch_data('15m', 15000))
    df_1h = add_all_indicators(engine.fetch_data('1h', 15000))
    ...
    print(f"✅ Data Ready: {len(sim_data)} rows (December 2025).")
    
    # Gem Sniper Ultra Limit
    limit = 52.75
    
    print(f"\n🔍 GEM SNIPER ULTRA (ATR < {limit}) - DECEMBER LOG")
            ...
            for f_idx, f_row in future_data.iterrows():
                ...
                if signal == 'LONG':
                    ...
                    if l <= sl: 
                        ...
                        pnl_amt = points * -1 # Loss logic correction: points is +ve distance, so * -1
                        break
                else:
                    if l <= tp: 
                        outcome = "WIN"
