...
def run_standard_simulation_jan8():
    print("💎 GEM SNIPER STANDARD (ATR < 60.00): JAN 8 ONLY")
    print("="*70)
    
    engine = TradingEngine()
    
    # 1. Fetch Data
    # Fetch enough data to cover Jan 8 with lookback
    ...
    # 2. Filter Date Range (Jan 8 Only)
    ...
    for idx, row in sim_data.iterrows():
        ...
        if atr_val > limit: 
             # Debug print for high ATR
             if idx.minute % 15 == 0: # Print occasionally
                 print(f"DEBUG: {idx} | ATR {atr_val:.1f} > {limit} (BLOCKED)")
             continue
        ...
        # Debug confidence
        if conf_long > 0.4 or conf_short > 0.4:
            print(f"DEBUG: {idx} | ATR {atr_val:.1f} | L_CONF {conf_long:.2f} | S_CONF {conf_short:.2f}")
    ...
    print("\n" + "="*70)
    print(f"📊 STANDARD VERIFICATION RESULTS (Jan 8)")
