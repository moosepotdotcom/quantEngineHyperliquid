...
def run_jan10_verification():
    print("💎 GEM SNIPER: JAN 10 VERIFICATION (Missed Trades)")
    print("="*70)
    
    engine = TradingEngine()
    # Fetch ample data to ensure we cover Jan 10
    ...
    for idx, row in sim_data.iterrows():
        ...
        if signal:
            ...
            if outcome != "OPEN":
                ...
                total_points += points
                print(f"{str(idx):<20} | {signal:<5} | {entry_price:<10.2f} | {tp:<10.2f} | {sl:<10.2f} | {atr_val:<6.1f} | {duration:<3}m | {outcome}")
