...
class HyperliquidTrader:
    ...
    def __init__(self, testnet: bool = True):
        ...
        # Account Settings
        self.leverage = int(os.getenv('LEVERAGE', '27'))
        self.target_tp_pct = 0.015 # TP 1.5%
        self.target_sl_pct = 0.008 # SL 0.8%
        self.max_position_size = float(os.getenv('MAX_POSITION_SIZE', '0.01'))  # Max BTC per trade
        
        # Safety limits
        self.daily_loss_limit = float(os.getenv('DAILY_LOSS_LIMIT', '10000.0'))
    ...
    def place_tp_sl_orders(self, symbol: str, size: float, tp_price: float, sl_price: float, is_long: bool = True):
        ...
        try:
            ...
            # Restored user-requested grouping
            print(f"   📤 Sending Bulk Order Request (Grouped): {orders}")
            ...
            if result and result.get('status') == 'ok':
                ...
                for status in statuses:
                    if isinstance(status, dict) and 'error' in status:
                         print(f"⚠️  Order Status Error: {status}")
    ...
    def execute_signal(self, signal: Dict) -> bool:
        ...
        if order:
            ...
            # 3. Place TP/SL orders on Hyperliquid (Hard SL/TP on Exchange)
