        ...
        try:
            while iteration < max_iterations:
                ...
                if signal or mtf_signal:
                    # --- GEM SNIPER ULTRA LOGIC ---
                    # 1. HARD ATR FILTER (The 100% Guarantee)
                    # We check ATR again here to be absolutely sure.
                    atr_val = latest.get('atr_14', 100)
                    GEM_ATR_LIMIT = 52.75
                    
                    if atr_val > GEM_ATR_LIMIT:
                         print(f"\n🛑 GEM SNIPER BLOCKED: ATR {atr_val:.2f} > {GEM_ATR_LIMIT} (Too Volatile for 100% Win Rate)")
                         return
                    
                    # 2. TREND FILTER DISABLED 
                    # (Gem Sniper relies on pure Mean Reversion in Low Volatility)
                    
                    if signal:
                        print(f"\n💎 GEM TRADE EXECUTED (Winner Hunter)")
                        self.execute_trade(signal)
                    
                    if mtf_signal:
                        print(f"\n💎 GEM TRADE EXECUTED (MTF Scalper)")
                        # Pass the valid mtf_signal
