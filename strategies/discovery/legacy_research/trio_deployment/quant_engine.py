...
class TradingEngine:
    ...
    def load_thresholds(self):
        ...
        try:
            ...
            # EMERGENCY OPTIMIZED Thresholds (Post-Reconstruction)
            # Raised from 34.12% → 50% after discovering 100% loss rate
            self.winner_threshold_long = 0.5000   # 50% for LONG (was 34.12%)
            self.winner_threshold_short = 0.4789  # 47.89% for SHORT (unchanged)
            self.mtf_threshold_long = 0.6500   # 65% for LONG (was 59.87%)
            self.mtf_threshold_short = 0.6891  # 68.91% for SHORT (unchanged)
