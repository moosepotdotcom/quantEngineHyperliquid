...
def debug_jan2_9():
    print("🕵️‍♂️ DIAGNOSTIC: JAN 2 - JAN 9 FULL ANALYSIS")
    print("="*100)
    ...
    for idx, row in sim_data.iterrows():
        ...
        if raw_signal:
            ...
            if status == "BLOCKED (ATR)":
                ...
                if outcome == "WIN": accepted_wins += 1
                if outcome == "LOSS": accepted_losses += 1
            
            # Print mostly focused on accepted or blocked
            # print(f"{str(idx):<20} | {raw_signal:<5} | {conf:<6.2f} | {atr_val:<6.1f} | {status:<20} | {outcome}")

    print("\n" + "="*100)
    print(f"📊 FULL WEEK EFFICIENCY (Jan 2 - Jan 9)")
    print("-" * 50)
    print(f"✅ ACCEPTED TRADES (The 100% Stats):")
    print(f"   Wins:   {accepted_wins}")
    print(f"   Losses: {accepted_losses}")
    print("-" * 50)
    print(f"🛡️ BLOCKED TRADES (The Bullet Proof Vest):")
    print(f"   Blocked Wins:   {blocked_wins}")
    print(f"   Blocked Losses: {blocked_losses}")
    
    total_blocked = blocked_wins + blocked_losses
    if total_blocked > 0:
        avoided_loss_ratio = blocked_losses / total_blocked
        print(f"   Efficiency: {avoided_loss_ratio:.1%} of filtered trades were BAD")
