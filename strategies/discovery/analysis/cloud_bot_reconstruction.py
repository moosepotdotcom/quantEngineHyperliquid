#!/usr/bin/env python3
"""
EXACT Cloud Bot Reconstruction - Jan 1-8, 2026
Uses actual prediction logs + applies optimized filters (revision 00016)
"""
import json
import os
from datetime import datetime, timedelta
import requests
import pandas as pd

# Optimized bot configuration (revision 00016)
THRESHOLDS = {
    'Winner Hunter (1H)': {'LONG': 0.5000, 'SHORT': 0.4789},
    'MTF Scalper (5M)': {'LONG': 0.6500, 'SHORT': 0.6891}
}

TP_PCT = 0.015
SL_PCT = 0.008

def get_historical_candles(start_time, end_time):
    """Get 1-minute BTC candles for outcome simulation"""
    url = "https://api.hyperliquid.xyz/info"
    payload = {
        "type": "candleSnapshot",
        "req": {
            "coin": "BTC",
            "interval": "1m",
            "startTime": int(start_time.timestamp() * 1000),
            "endTime": int(end_time.timestamp() * 1000)
        }
    }
    
    try:
        resp = requests.post(url, json=payload, timeout=10)
        data = resp.json()
        if data:
            df = pd.DataFrame(data)
            df['timestamp'] = pd.to_datetime(df['t'], unit='ms')
            df['close'] = df['c'].astype(float)
            df['high'] = df['h'].astype(float)
            df['low'] = df['l'].astype(float)
            return df
        return None
    except:
        return None

def simulate_outcome(entry_price, direction, entry_time):
    """Simulate TP/SL outcome"""
    end_time = entry_time + timedelta(hours=48)
    candles = get_historical_candles(entry_time, end_time)
    
    if candles is None or len(candles) == 0:
        return "OPEN", 0.0, None
    
    if direction == "LONG":
        tp = entry_price * (1 + TP_PCT)
        sl = entry_price * (1 - SL_PCT)
    else:
        tp = entry_price * (1 - TP_PCT)
        sl = entry_price * (1 + SL_PCT)
    
    for _, candle in candles.iterrows():
        if direction == "LONG":
            if candle['high'] >= tp:
                return "WIN", TP_PCT * 100, candle['timestamp']
            if candle['low'] <= sl:
                return "LOSS", -SL_PCT * 100, candle['timestamp']
        else:
            if candle['low'] <= tp:
                return "WIN", TP_PCT * 100, candle['timestamp']
            if candle['high'] >= sl:
                return "LOSS", -SL_PCT * 100, candle['timestamp']
    
    return "OPEN", 0.0, None

def get_market_regime_for_date(date):
    """Get market regime for a specific date based on known data"""
    # From our earlier analysis:
    # Jan 1-6: BULLISH (market was rising)
    # Jan 7-8: BEARISH (market dropped)
    if date <= datetime(2026, 1, 6):
        return "BULLISH"
    else:
        return "BEARISH"

def should_trade(direction, regime):
    """Check if trade aligns with trend"""
    if direction == "LONG" and regime != "BULLISH":
        return False
    if direction == "SHORT" and regime != "BEARISH":
        return False
    return True

print("="*80)
print("🚀 EXACT CLOUD BOT RECONSTRUCTION - Jan 1-8, 2026")
print("="*80)
print("\nConfiguration (Revision 00016):")
print("  ✅ Thresholds: 50%/65% (Winner Hunter / MTF Scalper)")
print("  ✅ Trend Filter: ACTIVE")
print("  ✅ Source: Actual prediction logs")
print()

all_trades = []

# Process each day
for day_offset in range(8):  # Jan 1-8
    day_date = datetime(2026, 1, 1) + timedelta(days=day_offset)
    day_str = day_date.strftime('%Y-%m-%d')
    date_file = day_date.strftime('%Y%m%d')
    
    print(f"\n{'='*80}")
    print(f"📅 {day_str}")
    print(f"{'='*80}")
    
    # Try to load prediction logs
    log_file = f'logs/trades/predictions_{date_file}.jsonl'
    
    if not os.path.exists(log_file):
        print(f"  No prediction log available")
        continue
    
    with open(log_file, 'r') as f:
        predictions = [json.loads(line) for line in f]
    
    print(f"  Loaded {len(predictions)} predictions")
    
    # Get market regime for this date
    market_regime = get_market_regime_for_date(day_date)
    print(f"  Market Regime: {market_regime}")
    
    day_trades = []
    
    # Process each prediction
    for pred in predictions:
        model = pred.get('model', '')
        conf = pred.get('confidence', 0)
        
        if model not in THRESHOLDS or conf == 0:
            continue
        
        # Determine direction based on confidence and threshold
        direction = None
        
        # Check if confidence passes threshold
        if conf >= THRESHOLDS[model]['SHORT']:
            direction = "SHORT"
        elif conf >= THRESHOLDS[model]['LONG']:
            direction = "LONG"
        
        if not direction:
            continue
        
        # Apply trend filter
        if not should_trade(direction, market_regime):
            continue
        
        # This signal passes all filters!
        timestamp = datetime.fromisoformat(pred['timestamp'])
        price = pred['market_data']['close']
        
        # Simulate outcome
        outcome, pnl, exit_time = simulate_outcome(price, direction, timestamp)
        
        trade = {
            'date': day_str,
            'entry_time': timestamp.isoformat(),
            'model': model,
            'direction': direction,
            'confidence': conf,
            'entry_price': price,
            'tp_price': price * (1 + TP_PCT) if direction == "LONG" else price * (1 - TP_PCT),
            'sl_price': price * (1 - SL_PCT) if direction == "LONG" else price * (1 + SL_PCT),
            'market_regime': market_regime,
            'trend_filter': 'PASSED',
            'outcome': outcome,
            'pnl': pnl,
            'exit_time': exit_time.isoformat() if exit_time else None
        }
        day_trades.append(trade)
        all_trades.append(trade)
    
    # Day summary
    if day_trades:
        wins = len([t for t in day_trades if t['outcome'] == 'WIN'])
        losses = len([t for t in day_trades if t['outcome'] == 'LOSS'])
        pnl = sum(t['pnl'] for t in day_trades)
        
        print(f"\n  📊 {len(day_trades)} trades executed:")
        print(f"     Wins: {wins} ✅ | Losses: {losses} ❌")
        print(f"     P&L: {pnl:+.2f}%")
        
        # Show first few trades
        for i, trade in enumerate(day_trades[:3], 1):
            emoji = "✅" if trade['outcome'] == 'WIN' else "❌"
            print(f"     {i}. {emoji} {trade['model']} {trade['direction']} @ ${trade['entry_price']:,.0f} → {trade['outcome']}")
        
        if len(day_trades) > 3:
            print(f"     ... and {len(day_trades) - 3} more")
    else:
        print(f"  No trades (signals filtered by trend filter)")

# Final summary
print(f"\n{'='*80}")
print("📈 WEEK SUMMARY - Optimized Bot (Revision 00016)")
print(f"{'='*80}")

if all_trades:
    wins = [t for t in all_trades if t['outcome'] == 'WIN']
    losses = [t for t in all_trades if t['outcome'] == 'LOSS']
    
    print(f"\nTotal Trades: {len(all_trades)}")
    print(f"  Wins: {len(wins)} ✅")
    print(f"  Losses: {len(losses)} ❌")
    if len(wins) + len(losses) > 0:
        print(f"  Win Rate: {len(wins)/(len(wins)+len(losses))*100:.1f}%")
    print(f"  Total P&L: {sum(t['pnl'] for t in all_trades):+.2f}%")
    print(f"  Avg P&L/Trade: {sum(t['pnl'] for t in all_trades)/len(all_trades):+.2f}%")
    
    # Breakdown by day
    print(f"\n📊 Daily Breakdown:")
    for day_offset in range(8):
        day_date = datetime(2026, 1, 1) + timedelta(days=day_offset)
        day_str = day_date.strftime('%Y-%m-%d')
        day_trades = [t for t in all_trades if t['date'] == day_str]
        if day_trades:
            day_pnl = sum(t['pnl'] for t in day_trades)
            print(f"  {day_str}: {len(day_trades)} trades, {day_pnl:+.2f}% P&L")
    
    # Save
    with open('CLOUD_BOT_RECONSTRUCTION.json', 'w') as f:
        json.dump(all_trades, f, indent=2, default=str)
    
    print(f"\n✅ Saved to CLOUD_BOT_RECONSTRUCTION.json")
else:
    print("\n❌ No trades found")

print(f"\n{'='*80}")
print("✅ Cloud bot reconstruction complete!")
print(f"{'='*80}")
