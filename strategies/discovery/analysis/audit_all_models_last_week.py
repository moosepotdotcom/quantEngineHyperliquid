#!/usr/bin/env python3
"""
📊 MULTI-MODEL WEEKLY AUDIT
Simulates all three models (Winner Hunter, MTF Scalper, Gem Sniper) 
on live Hyperliquid data from the last 8 days.
"""
import os
import sys
import pandas as pd
...
class WeeklyAudit:
    ...
    def run_audit(self):
        print("\n" + "🚀"*20)
        print("📊 MULTI-MODEL WEEKLY AUDIT")
        print("🚀"*20)
        ...
        configs = [
            ...
            {'name': 'MTF Scalper (5M)', 'target': 'mtf', 'thresh': 0.9584, 'tp': 0.015, 'sl': 0.008},
            {'name': 'Gem Sniper (5M)',  'target': 'mtf', 'thresh': 0.90,   'tp': 0.003, 'sl': 0.005}
        ]
        ...
        for cfg in configs:
            ...
            for idx in signals:
                ...
                outcome = "OPEN"
                # Use df_5m_raw for high-resolution outcome verification
                df_verify = df_5m_raw[df_5m_raw['timestamp'] >= entry_time]
                
                for _, row in df_verify.iterrows():
                    if row['high'] >= tp_price: 
                        outcome = "WIN"
                        # Find the index in df that corresponds to this resolution
                        # This ensures we don't start a new trade while one is open
                ...
                if outcome != "OPEN":
                    all_trades.append({
                        ...
                        'PnL $': (cfg['tp'] if outcome == "WIN" else -cfg['sl']) * 1.27 * 95000 
                    })

        # 4. Report
        report = pd.DataFrame(all_trades)
        if report.empty:
            print("No trades found for any model.")
            return
            
        report.sort_values('Time', inplace=True)
        print("\n" + "="*80)
        print("📝 CONSOLIDATED WEEKLY TRADE LOG")
        print("="*80)
